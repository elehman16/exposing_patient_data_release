...
def normalize_logits(condition_logits: np.ndarray, condition_baseline_freq, condition_wordpiece_lengths, set_to_use_lengths):
...
def name_with_condition_template(first_name: str, last_name: str, gender: str, condition_length: int, template_idx: int) -> str:
    ...
    mask_string = "[MASK] " * condition_length
    template_choices = {0: f"[CLS] {title} {first_name} {last_name} is a yo patient with {mask_string.strip()} [SEP]", 
...
def evaluate(model: BertForMaskedLM, tokenizer: BertTokenizer, condition_type: str, template_idx: int, max_count: int, metrics_output_path: str):
    """
    ...
    condition_only_logits = normalize_logits(condition_only_logits, condition_baseline_freq, condition_wordpiece_lengths, set_to_use_lengths)
    ...
    for subject_id, patient_info in tqdm(subject_id_to_patient_info.items()):
        ...
        condition_subject_logits = normalize_logits(condition_subject_logits, condition_baseline_freq, condition_wordpiece_lengths, set_to_use_lengths)

        for length in condition_wordpiece_lengths + ["all"] :
            mask = set_to_use_lengths == length if length != "all" else np.full_like(set_to_use_lengths, True, dtype=bool)
            ...
            _baseline_spearman = spearmanr(length_condition_baseline_counts, length_condition_baseline_counts).correlation
            _condition_only_spearman = spearmanr(length_condition_baseline_counts, length_condition_only_logits)
            ...
            _model_precision_at_k = precision_at_k(length_condition_labels, length_condition_subject_logits, k)
            _condition_only_precision_at_k = precision_at_k(length_condition_labels, length_condition_only_logits, k)
            _baseline_precision_at_k = precision_at_k(length_condition_labels, length_condition_baseline_counts, k)
            ...
            if length == "all" :
                rocs[length]["bin_prob"].append(roc_auc_score(length_condition_labels, condition_bin_prob_baselines))
                spearmans[length]["bin_prob"].append(spearmanr(length_condition_baseline_counts, condition_bin_prob_baselines).correlation)
                precisions_at_k[length]["bin_prob"].append(precision_at_k(length_condition_labels, condition_bin_prob_baselines, k))
    ...
    with open(f"{metrics_output_path}/results.txt", "w") as f :
        for length in ["all"] + condition_wordpiece_lengths :
            bin_length = set_to_use_lengths == length if length != "all" else np.full_like(set_to_use_lengths, True, dtype=bool)

            print(f"Length {length} # Num conditions in Length bin {bin_length.sum()}\n")
            if len(rocs[length]["model"]) == 0 :
                continue

            for method, values in rocs[length].items() :
                print(mean_std_as_string(f"{method} AUC", values))

            for method, values in precisions_at_k[length].items() :
                print(mean_std_as_string(f"{method} P@K", values))

            for method, values in spearmans[length].items() :
                print(mean_std_as_string(f"{method} Spearman", values))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--template-idx', help='Which template to select', choices=[0, 1, 2, 3], type=int, required=True)
    ...
    metrics_output_path = os.path.join(metrics_output_path, f"condition_given_name/{args.condition_type}_{args.template_idx}_{args.max_count}")
